<?php
	...
	{
		$idP = $_GET["idP"];
		$quantidadeE = $_GET["quantidadeE"];
		$dataE = $_GET["dataE"];

		$pdo = new PDO('mysql:host=localhost;dbname=sistemaproduto;charset=utf8', "root", "");
		$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$sql = "insert into entrada (idP, quantidadeE, dataE)  Values(?,?,?)";
		$query = $pdo->prepare($sql);
		$query->execute(array($idP,$quantidadeE,$dataE));
		...
		$quantidadeP = (int)$quantidadeE + (int)$quantidadeP ;

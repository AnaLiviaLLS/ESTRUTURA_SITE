<?php
	...
	if (isset($_SESSION["Logado"]))
	{
		...
		if ($quantidadeP >= $quantidadeS)
		{
			
			$sql = "insert into saida (idP, quantidadeS, dataS)  Values(?,?,?)";
			$query = $pdo->prepare($sql);
			$query->execute(array($idP,$quantidadeS,$dataS));
			
			$quantidadeP = (int)$quantidadeP - (int)$quantidadeS;
		...
		{
			echo"<script>alert('A quantidade de produto no estoque = ".$quantidadeP." é menor que a quantidade de saida = ".$quantidadeS." ')</script>)";

	...
	<table class="table table-hover text-center table-dark">
		<thead>
			<tr>
				...
				<th> Quantidade </th>
				<th> Data </th>
				<th class="text-danger"> Entrada </th>
			</tr>
		</thead>
...
<?php
	...
	if (isset($_SESSION["Logado"]))
	{
		...
		$sqlTotal = $pdo->prepare("select * from entrada order by dataE desc");
		$sqlTotal ->execute();
		...
		$sqlLimit = $pdo->prepare("select * from entrada order by dataE desc limit $exibirQtsPag, $exibirQts");
		$sqlLimit->execute();
		...
		if($sqlTotal->rowCount()>0)
		{
			for($j=0;$j < count($resultLimit);$j++)
			{
				...
				echo "<td><a title='Deletar' href='deletarE.php?idE=".$resultLimit[$j][0]."'><img src='imagem/delete.jpg' width='17' height='17'></a></td>";  
				echo "</tr>";
				$numero++;
			}
			echo "</tbody>";

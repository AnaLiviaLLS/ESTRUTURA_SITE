<?php
	...
	if (isset($_SESSION["Logado"]))
	{
		...
		if($precoC < $precoV )
		{
			...
			$sql = "insert into Produto (idP, nomeP,sexoP, idTipo, idMarca,tamanhoP, quantidadeP, precoC, precoV, assunto,fornecedor)  Values(?,?,?,?,?,?,?,?,?,?,?)";
			$query = $pdo->prepare($sql);
			$query->execute(array($idP,$nomeP,$sexoP,$idTipo,$idMarca,$tamanhoP,$quantidadeP,$precoC,$precoV,$assunto,$fornecedor));
			$pdo = null;
		...
		{
			echo"<script>alert('Cadastro Não pode ser realizado pois o preco de compra = $precoC é maior que o preço de venda = $precoV !!');</script>";
			echo"<script>window.location='cadastrarP.php';</script>";
		}
	}
	else
	{
